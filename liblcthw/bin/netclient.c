...
int nonblock(int fd)
{
    check(fd >= 0, "Invalid file descriptot passed to nonblock.");
...
{
    check(host, "Host can't be NULL.");
    check(port, "Port can't be NULL.");
...
{
    check(buffer, "Buffer passed to read_some can't be NULL.");
    check(fd >= 0, "Invalid file descriptor passed to read_some.");
...
int write_some(RingBuffer * buffer, int fd, int is_socket, int no_translate)
{
    check(buffer, "Buffer passed to write_some can't be NULL.");
    check(fd >= 0, "Invalid file descriptor passed to write_some.");
...
int main(int argc, char *argv[])
{
    ...
    check((argc == 3) || (argc == 4), "USAGE: netclient host port [-n]");

    int no_translate = 0;
    no_translate = getopt(argc, argv, "n");
	...
	while (!RingBuffer_empty(sock_rb)) {
	    rc = write_some(sock_rb, 1, 0, no_translate);
	    check_debug(rc != -1, "Failed to write to stdout.");
	}

	while (!RingBuffer_empty(in_rb)) {
	    rc = write_some(in_rb, socket, 1, no_translate);

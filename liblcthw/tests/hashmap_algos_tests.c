...
enum { ALGO_FNV1A, ALGO_ADLER32, ALGO_DJB, ALGO_BAD, ALGO_JENKINS, ALGO_ELF };
...
char *test_distribution()
{
    ...
    fill_distribution(stats[ALGO_ELF], keys, Hashmap_elf_hash);

    fprintf(stderr, "FNV\tA32\tDJB\tBAD\tJEN\tELF\n");

    for (i = 0; i < BUCKETS; i++) {
        fprintf(stderr, "%d\t%d\t%d\t%d\t%d\t%d\n",
		stats[ALGO_FNV1A][i],
		stats[ALGO_ADLER32][i], stats[ALGO_DJB][i],
		stats[ALGO_BAD][i], stats[ALGO_JENKINS][i],
		stats[ALGO_ELF][i]);
    }

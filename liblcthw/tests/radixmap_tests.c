...
static char *test_operations()
{
    ...
    debug("Adding and sorting %u random keys took %1.10f seconds", N, seconds);
    ...
    uint32_t insert = map->contents[100].data.key;
    RadixMap_add(map, insert, 1);
    int res = RadixMap_closest(map, insert);
    mu_assert(res >= 99, "Wrong closest number.");

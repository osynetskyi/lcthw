...
int write_scan(const char *fmt, ...)
{
    ...
    for (i = 0; fmt[i] != '\0'; i++) {
		...
		case 'd':
		    out_int = va_arg(argp, long *);
		    out_string = calloc(1, sizeof(char *));
		    *out_string = calloc(1, sizeof(char) * 15);

		    snprintf(*out_string, 15, "%ld", *out_int);
...
int main(int argc, char *argv[])
{
    ...
    write_scan("---- RESULTS ----\n");
    write_scan("First Name: %s\n", &first_name);
    write_scan("Initial: '%c'\n", &initial);
    write_scan("Last Name: %s\n", &last_name);
    write_scan("Age: %d\n", &age);
